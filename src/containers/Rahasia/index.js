...
import {
  ...
  StatByDay as StatByDayAction,
  CountSubmitted as CountSubmittedAction,
} from '../../api';
...
class Rahasia extends Component {
  constructor(props) {
    super(props);
  }
  ...
  componentWillMount = () => {
    ...
    CountSubmittedAction(content)
      .then(res => {
        console.log("baru: ", res);
        const registered = res.registered
        const submited = res.submited
        this.setState({
          allSubmitted: submited,
          allRegistered: registered,
        })
      })

    StatByDayAction(content)
      .then(res => {
        console.log("res stat baruuuuuuu: ", res);
        const reg = res.registered
        const sub = res.submited
        const newRegistered = Object.keys(reg).map(i => reg[i])
        const newSubmitted = Object.keys(sub).map(i => sub[i])
        const dateNew = Object.keys(sub).map(i => i)
        this.setState({
          statRegistered: newRegistered,
          statSubmited: newSubmitted,
          dateNew,
        })
  ...
  render() {
    ...
    console.log("['x', ...dateNew]: ", ['x', ...dateNew]);
    console.log("allSubmitted: ", allSubmitted);
    console.log("allRegistered: ", allRegistered);
    // const chartData = {
    //   line: {
    //     Registered: statRegistered,
    //     Submitted: statSubmited,
    //   },
    // };
    ...
    console.log("showWilayahhhhhssssssss: ", showWilayah);
    return (
      <div className={styles}>
        ...
        {
          dateNew.length > 0 && <C3Chart data={dataChart} grid={gridData} />
        }

        <C3Chart data={{ json: barChartData.bar, type: 'bar' }} />;
...
export default connect(mapStateToProps, mapDispatchToProps)(Rahasia);
